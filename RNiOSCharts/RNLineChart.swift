...
class RNLineChart : LineChartView {
  ...
  func setChart() {
    ...
    let chartDataSet = LineChartDataSet(yVals: dataEntries, label: self.dataSetLabel);
    chartDataSet.drawCirclesEnabled = false;
    chartDataSet.lineWidth = CGFloat(2);
    
    let chartData = LineChartData(xVals: self.labels, dataSet: chartDataSet);
    self.data = chartData;
